...
class MinimaxAI(AIAlgorithm):
    ...
    def load_transposition_table(self, chess_state: Tuple[Tuple[int, int], int]) -> None:
        ...
        self.transposition_file = Path(f"{self.transposition_path}/transposition_table({row_len}_{col_len}&{power})(sha256).json")
        ...
        try:
            if self.transposition_file.exists():
                with open(self.transposition_file, "r", encoding="utf-8") as file:
                    ...
                    logger.info(f"Loaded transposition table: {self.transposition_file}") if self.log_mode else None
            else:
                # 创建空 json
                with open(self.transposition_file, "w", encoding="utf-8") as file:
                    json.dump({}, file, indent=4)
        except json.JSONDecodeError:
            logger.warning(f"Transposition table corrupt, resetting: {self.transposition_file}")
            with open(self.transposition_file, "w", encoding="utf-8") as file:
                json.dump({}, file, indent=4)
    ...
    def save_transposition_table(self) -> None:
        ...
        logger.info(f"Saved transposition table to {self.transposition_file}") if self.log_mode else None
