...
class MCTSPlusNode:
    def __init__(
        ...
        parent: Optional["MCTSPlusNode"] = None,
        root_color: Optional[int] = None,
    ):
        self.chess_game = chess_game  # 当前节点的游戏状态
        self.parent: MCTSPlusNode = parent  # 父节点
        self.root_color = root_color  # 目标颜色

        self.children: List[MCTSPlusNode] = []  # 子节点列表
        self.visits = 0  # 访问次数
        self.wins = 0  # 胜利次数
    ...
    def expand(self):
        ...
        child_node = MCTSPlusNode(new_chess_game, parent=self, root_color=self.root_color)
...
class MCTSPlusAI(BaseAI):
    ...
    def tree_policy_with_net(
        self, node: MCTSPlusNode, policy_net: Optional[DeepLearningAI], c_param
    ) -> MCTSPlusNode:
