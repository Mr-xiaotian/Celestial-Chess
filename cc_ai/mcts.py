...
class MCTSNode:
    ...
    def get_best_child_with_net(self, policy_net: Optional[DeepLearningAI], c_param=0.6):
        """
        使用UCB1策略选择最佳子节点
        """
...
class MCTSAI(AIAlgorithm):
    ...
    def find_best_move(self, game: ChessGame) -> Tuple[int, int]:
        ...
        # 如果是完整模式，则更新游戏状态和评分板
        self.update_game_with_mcts_results(game, root, best_child) if self.complete_mode else None

        return best_move
    
    def update_game_with_mcts_results(self, game: ChessGame, root: MCTSNode, best_child: MCTSNode) -> None:
        """
...
class MCTSPlusAi(MCTSAI):
    ...
    def MCTS(self, root: MCTSNode) -> MCTSNode:
        ...
        for _ in range(self.itermax):
            node = self.tree_policy_with_net(root, policy_net=self.policy_net, c_param=self.c_param)
            reward = node.simulate()
            node.backpropagate(reward)
        return root.get_best_child(c_param=0)
    
    def tree_policy_with_net(self, node: MCTSNode, policy_net: Optional[DeepLearningAI], c_param) -> MCTSNode:
        """根据选择策略递归选择子节点，直到达到未完全展开或未被访问的节点"""
        while not node.chess_game.is_game_over():
            if not node.is_fully_expanded():
                return node.expand()
            else:
                node = node.get_best_child_with_net(policy_net=policy_net, c_param=c_param)
        return node